...
namespace DAL
{
    public class Activity
    {
        ...
        public IEnumerable<Model.Activity> Getnew()
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select * from activityVerify='审核通过' and endTime>=now() order by endTime>=now() order by endTime desc limit 2";
            }
        }
        public Model.Activity GetRecommend()
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select * from activityVerify='审核通过' and recommend='是' and endTime>=now() order by recommendTime desc limit 1";
        ...
        public IEnumerable<Model.ActivityName> GetActivityNames()
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select sctivityId,activityName from activity";
        ...
        public IEnumerable<Model.ActivityNo> GetPage(Model.Page page)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                ...
                sql += "select* froma where num between (@pageIndex-1)*@pageSize+1 and @pageIndex*@pageSize;";
